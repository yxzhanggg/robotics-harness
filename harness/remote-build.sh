...
build_device() {
  ...
  ssh_bash "${host}" "set -euo pipefail; ${ros_setup} cd ${remote_ws_expr}; rosdep install --from-paths src --ignore-src -y; ${makeflags} colcon build --symlink-install ${worker_args} ${package_args}"
